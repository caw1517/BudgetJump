-- Link each paycheck to the account that receives the deposit (net pay), and keep that account balance in sync.

alter table public.paychecks
add column if not exists deposit_account_id uuid references public.financial_accounts(id) on delete restrict;

create index if not exists idx_paychecks_deposit_account_id on public.paychecks(deposit_account_id);
...
begin
  ...
  values (p_date, trim(p_source), p_net_amount_cents, nullif(trim(p_notes), ''), p_deposit_account_id)
